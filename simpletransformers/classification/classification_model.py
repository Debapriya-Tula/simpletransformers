...
from simpletransformers.classification.mixup import MIXUP
...
class ClassificationModel:
    ...
    def eval_model(
        self, eval_df, processed_df, lang, multi_label=False, output_dir=None, verbose=True, silent=False, wandb_log=True, **kwargs
    ):
        """
        ...
        result, model_outputs, wrong_preds = self.evaluate(
            eval_df, processed_df, lang, output_dir, multi_label=multi_label, verbose=verbose, silent=silent, wandb_log=wandb_log, **kwargs
        )
    ...
    def evaluate(
        self, eval_df, processed_df, lang, output_dir, multi_label=False, prefix="", verbose=True, silent=False, wandb_log=True, **kwargs
    ):
        """
    ...
    def load_and_cache_examples(
        ...
        if args.sliding_window or self.args.model_type == "layoutlm":
            ...
            # logger.critical(f'{dataset.examples}')
            return dataset
    ...
    def predict(self, to_predict, processed_df, lang, multi_label=False):
        """
    ...
    def _get_inputs_dict(self, batch, no_hf=False, processed_df=None, base_lang=None, use_mixup=False):
        ...
        if isinstance(batch[0], dict):
            ...
            if use_mixup:
                ...
                input_ids = batch[0]
                mixup = MIXUP()
                generator, step = mixup.flow(data=input_ids, labels=batch[3])
