...
def calculate_cmi(input_df, lang):
    ...
    output_df['Input'] = input_df['Input'].apply(
        lambda text: lit.identify(text)
    )
...
class CustomLoss(nn.Module):
    ...
    def forward(self, input, target, processed_input_df, base_lang):
        cmi_list = list(map(lambda x : utteranceCMI(x, base_lang), processed_input_df['Input']))
        cmi_mean = np.array(cmi_list).mean()

        # LW=α∗CE∗(1−β)γ+α∗CE∗βγ
        # CMI instead of β
        term1 = self.alpha * F.cross_entropy(input, target) * (1 - cmi_mean)**self.gamma
